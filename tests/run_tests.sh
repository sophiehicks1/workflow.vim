...
for module_file in $TEST_MODULE_FILES; do
  ...
  # create a temp vimrc to set up environment
  vimrc="$TEST_TEMP_DIR/vimrc"
  echo "let g:test_module_name=\"$module_name\"" >> "$vimrc"
  echo "let g:test_module_file=\"$module_file\"" >> "$vimrc"
  echo "let g:test_temp_dir=\"$TEST_TEMP_DIR\"" >> "$vimrc"
  echo "let g:test_function_filter=\"$FUNCTION_FILTER\"" >> "$vimrc"

  # Source all autoload files to make plugin functions available for testing
  for file in `find "autoload/"`
  do
    if [[ "$file" == *.vim ]]; then
